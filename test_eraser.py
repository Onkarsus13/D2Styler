...
def make_prediction(file_, text):
        ...
        text = text.split(" ")[3]
        crr = CraftReader('./img.png')
        crnn=CRNNReader()
        boxes, img_res = crr.boxes_detect()
        zeros = np.zeros((512, 512, 3))
        print(img_res.shape)
        for _, tmp_box in enumerate(boxes):
            ...
            zeros[y:y+h, x:x+w, :] = 255
            z_i = Image.fromarray(zeros.astype('uint8')).convert('RGB')
            ...
            result = crnn.get_predictions(tmp_img)
            print(result)
            
            if result == text.lower().replace("'", ""):
                ...
                image.resize(shape).save('test1.png')
                break
            else:
                zeros = np.zeros((512, 512, 3))
...
class DiffusionTransformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.content_seq_len = 1024
        self.amp = False

        self.num_classes = 4097
    ...
    def sample_time(self, b, device, method='uniform'):
        if method == 'importance':
            ...
            t = torch.multinomial(pt_all, num_samples=1, replacement=True)
            t = torch.zeros((b,)) + t
...
make_prediction("/home/awd8324/onkar/Diff_SceneTextEraser/I1.png", "remove the word 'zahours' which is above 'GLEN'")
